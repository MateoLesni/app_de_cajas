// static/js/once.js
// Last updated: 2025-06-26 02:32:00
// Current user: pomenuk

document.addEventListener('DOMContentLoaded', function() {
    // Constantes y variables globales
    const ETIQUETAS_ONCE = [
        "HO03", "HO04", "HO07", "HO13", "HO19", "HO21",
        "HO22", "HO33", "HO43", "HO44", "HO46", "HO47",
        "HO48", "HO50", "HO54", "HO55"
    ];
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('onceFixedDate');
        const isFixed = localStorage.getItem('onceDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        // Agregar a initializeComponents() en once.js
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    ...
                    const response = await fetch(`/api/once/last_update?timestamp=${lastUpdate}`);
            ...
            async function checkFileStatus() {
                try {
                    const response = await fetch('/api/once/file_status');
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_ONCE
            ETIQUETAS_ONCE.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('onceFixedDate', currentControlDate);
            localStorage.setItem('onceDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('onceFixedDate');
            localStorage.setItem('onceDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de once sin filtros - para verificar todos los registros
            const onceResponse = await fetch(`/api/once/data`);
            if (!onceResponse.ok) {
                throw new Error('Error al cargar los datos de Once');
            }
            const onceData = await onceResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de once
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/once?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona once
            pendingData = pendingData.filter(item => item.zona === 'ONCE');
            ...
            for (const etiqueta of ETIQUETAS_ONCE) {
                const turnos = ETIQUETA_TURNOS_MAP[etiqueta] || ["UNI"]; // Usar los turnos mapeados o UNI por defecto
                
                for (const turno of turnos) {
                    // Buscar registros OK en once.json
                    const registrosOK = onceData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/once/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/once/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/once/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/once/update', {
                method: 'POST',
                headers: {
