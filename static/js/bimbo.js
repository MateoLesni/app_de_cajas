...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const ETIQUETAS_BIMBO = [
        "UFB07","UFB11","UFB53","UFB61","UFB69","UFB86","UFB92", "UFB106", "UFB123", "UFB124", "UFB130", "UFB132", 
        "HL01", "SH18", "UFB31", "UFB32", "UFB33", "UFB50", 
        "UFB55", "UFB60", "UFB82", "UFB110", "UFB127", "ZJ02", 
        "ZT60", "UFB12", "UFB13", "UFB89", "UFB01", "UFB03", 
        "UFB52", "UFB93", "UFB128", "UFB129", "HO25", "UFB133", "UFB134", "UFB09"
    ];
    
    // Crear mapeo de etiquetas a turnos permitidos (todas son UNI)
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('bimboFixedDate');
        const isFixed = localStorage.getItem('bimboDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        // Agregar a initializeComponents() en bimbo.js
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    ...
                    const response = await fetch(`/api/bimbo/last_update?timestamp=${lastUpdate}`);
            ...
            async function checkFileStatus() {
                try {
                    const response = await fetch('/api/bimbo/file_status');
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_BIMBO
            ETIQUETAS_BIMBO.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('bimboFixedDate', currentControlDate);
            localStorage.setItem('bimboDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('bimboFixedDate');
            localStorage.setItem('bimboDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de bimbo sin filtros - para verificar todos los registros
            const bimboResponse = await fetch(`/api/bimbo/data`);
            if (!bimboResponse.ok) {
                throw new Error('Error al cargar los datos de bimbo');
            }
            const bimboData = await bimboResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de bimbo
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/bimbo?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona bimbo
            pendingData = pendingData.filter(item => item.zona === 'BIMBO');
            ...
            for (const etiqueta of ETIQUETAS_BIMBO) {
                const turnos = ETIQUETA_TURNOS_MAP[etiqueta] || ["UNI"]; // Usar los turnos mapeados o UNI por defecto
                
                for (const turno of turnos) {
                    // Buscar registros OK en bimbo.json
                    const registrosOK = bimboData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/bimbo/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/bimbo/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/bimbo/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/bimbo/update', {
                method: 'POST',
                headers: {
