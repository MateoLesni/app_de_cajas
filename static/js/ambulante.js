// static/js/ambulante.js
// Last updated: 2025-06-25 20:15:00
// Current user: pomenuk

document.addEventListener('DOMContentLoaded', function() {
    // Constantes y variables globales
    const ETIQUETAS_AMBULANTE = ["VA02", "VA03", "VA04", "VA05"];
    const TURNOS_AMBULANTE = ["UNI"];
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('ambulanteFixedDate');
        const isFixed = localStorage.getItem('ambulanteDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    // Obtener un hash o timestamp del último cambio - CORREGIDO: usar ambulante en lugar de zona_sur
                    const response = await fetch(`/api/ambulante/last_update?timestamp=${lastUpdate}`);
                    const data = await response.json();
                    
                    console.log("Polling ambulante:", data); // Para depuración
                    ...
                    console.error("Error verificando actualizaciones de ambulante:", error);
                }
            }
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_AMBULANTE
            ETIQUETAS_AMBULANTE.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('ambulanteFixedDate', currentControlDate);
            localStorage.setItem('ambulanteDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('ambulanteFixedDate');
            localStorage.setItem('ambulanteDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de ambulante sin filtros - para verificar todos los registros
            const ambulanteResponse = await fetch(`/api/ambulante/data`);
            if (!ambulanteResponse.ok) {
                throw new Error('Error al cargar los datos de ambulante');
            }
            const ambulanteData = await ambulanteResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de ambulante
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/ambulante?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona ambulante
            pendingData = pendingData.filter(item => item.zona === 'AMBULANTE');
            ...
            // Usar solo las etiquetas VA02, VA03, VA04, VA05
            for (const etiqueta of ETIQUETAS_AMBULANTE) {
                for (const turno of TURNOS_AMBULANTE) {
                    // Buscar registros OK en ambulante.json
                    const registrosOK = ambulanteData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/ambulante/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/ambulante/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/ambulante/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/ambulante/update', {
                method: 'POST',
                headers: {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const response = await fetch(`/api/ambulante/delete/${id}`, {
                        method: 'DELETE'
                    });
