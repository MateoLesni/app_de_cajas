// static/js/retiro.js
// Last updated: 2025-06-26 02:40:27
// Current user: pomenuk

document.addEventListener('DOMContentLoaded', function() {
    // Constantes y variables globales
    const ETIQUETAS_RETIRO = [
        "JM07", "JM43", "JM45", "JM60", "CC11", 
        "CC12", "ZT51", "CR13", "CR14"
    ];
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('retiroFixedDate');
        const isFixed = localStorage.getItem('retiroDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        // Agregar a initializeComponents() en retiro.js
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    ...
                    const response = await fetch(`/api/retiro/last_update?timestamp=${lastUpdate}`);
            ...
            async function checkFileStatus() {
                try {
                    const response = await fetch('/api/retiro/file_status');
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_RETIRO
            ETIQUETAS_RETIRO.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('retiroFixedDate', currentControlDate);
            localStorage.setItem('retiroDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('retiroFixedDate');
            localStorage.setItem('retiroDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de retiro sin filtros - para verificar todos los registros
            const retiroResponse = await fetch(`/api/retiro/data`);
            if (!retiroResponse.ok) {
                throw new Error('Error al cargar los datos de Retiro');
            }
            const retiroData = await retiroResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de retiro
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/retiro?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona retiro
            pendingData = pendingData.filter(item => item.zona === 'RETIRO');
            ...
            for (const etiqueta of ETIQUETAS_RETIRO) {
                const turnos = ETIQUETA_TURNOS_MAP[etiqueta] || ["UNI"]; // Usar los turnos mapeados o UNI por defecto
                
                for (const turno of turnos) {
                    // Buscar registros OK en retiro.json
                    const registrosOK = retiroData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/retiro/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/retiro/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/retiro/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/retiro/update', {
                method: 'POST',
                headers: {
