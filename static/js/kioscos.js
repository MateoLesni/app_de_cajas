// static/js/kioscos.js
// Last updated: 2025-06-25 20:30:00
// Current user: pomenuk

document.addEventListener('DOMContentLoaded', function() {
    // Constantes y variables globales
    const ETIQUETAS_KIOSCOS = [
        "KO01", "KO02", "KO04", "KO05", "KO11", "KO10", 
        "KF01", "KM00", "KM04", "KZ02", "KC01", "KR01"
    ];
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('kioscosFixedDate');
        const isFixed = localStorage.getItem('kioscosDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        // Agregar a initializeComponents() en kioscos.js
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    ...
                    const response = await fetch(`/api/kioscos/last_update?timestamp=${lastUpdate}`);
            ...
            async function checkFileStatus() {
                try {
                    const response = await fetch('/api/kioscos/file_status');
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_KIOSCOS
            ETIQUETAS_KIOSCOS.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('kioscosFixedDate', currentControlDate);
            localStorage.setItem('kioscosDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('kioscosFixedDate');
            localStorage.setItem('kioscosDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de kioscos sin filtros - para verificar todos los registros
            const kioscosResponse = await fetch(`/api/kioscos/data`);
            if (!kioscosResponse.ok) {
                throw new Error('Error al cargar los datos de Kioscos');
            }
            const kioscosData = await kioscosResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de kioscos
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/kioscos?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona kioscos
            pendingData = pendingData.filter(item => item.zona === 'KIOSCOS');
            ...
            for (const etiqueta of ETIQUETAS_KIOSCOS) {
                const turnos = ETIQUETA_TURNOS_MAP[etiqueta] || ["UNI"]; // Usar los turnos mapeados o UNI por defecto
                
                for (const turno of turnos) {
                    // Buscar registros OK en kioscos.json
                    const registrosOK = kioscosData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/kioscos/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/kioscos/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/kioscos/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/kioscos/update', {
                method: 'POST',
                headers: {
