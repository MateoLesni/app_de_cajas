// static/js/tba.js
// Last updated: 2025-06-26 02:49:15
// Current user: pomenuk

document.addEventListener('DOMContentLoaded', function() {
    // Constantes y variables globales
    const ETIQUETAS_TBA = [
        "MJ15", "MJ16", "MJ22", "MJ23", "MJ24", "MJ35", 
        "BR03", "CR08", "CR05", "LI00", "LI01", "LI05", 
        "LI07", "LI09", "LI11"
    ];
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('tbaFixedDate');
        const isFixed = localStorage.getItem('tbaDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        // Agregar a initializeComponents() en tba.js
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    ...
                    const response = await fetch(`/api/tba/last_update?timestamp=${lastUpdate}`);
            ...
            async function checkFileStatus() {
                try {
                    const response = await fetch('/api/tba/file_status');
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_TBA
            ETIQUETAS_TBA.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('tbaFixedDate', currentControlDate);
            localStorage.setItem('tbaDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('tbaFixedDate');
            localStorage.setItem('tbaDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de tba sin filtros - para verificar todos los registros
            const tbaResponse = await fetch(`/api/tba/data`);
            if (!tbaResponse.ok) {
                throw new Error('Error al cargar los datos de TBA');
            }
            const tbaData = await tbaResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de tba
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/tba?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona tba
            pendingData = pendingData.filter(item => item.zona === 'TBA');
            ...
            for (const etiqueta of ETIQUETAS_TBA) {
                const turnos = ETIQUETA_TURNOS_MAP[etiqueta] || ["UNI"]; // Usar los turnos mapeados o UNI por defecto
                
                for (const turno of turnos) {
                    // Buscar registros OK en tba.json
                    const registrosOK = tbaData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/tba/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/tba/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/tba/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/tba/update', {
                method: 'POST',
                headers: {
