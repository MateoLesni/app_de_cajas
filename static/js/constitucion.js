// static/js/constitucion.js
// Last updated: 2025-06-25 19:19:00
// Current user: pomenuk

document.addEventListener('DOMContentLoaded', function() {
    // Constantes y variables globales
    const ETIQUETAS_CONSTITUCION = [
        "JM12", "JM13", "JM21", "JM22", "JM23", "JM25", "JM28", "JM32", "JM33", 
        "JM39", "JM42", "TH07", "TH13", "TH54", "PF00", "PF02", "PF03", "PF04", 
        "PF05", "PF06", "CR33", "SH04", "SH11", "BA04", "BA05", "BA06", "BA07"
    ];
    ...
    function initializeComponents() {
        ...
        const savedDate = localStorage.getItem('constitucionFixedDate');
        const isFixed = localStorage.getItem('constitucionDateFixed') === 'true';
        dateFixed = isFixed;
        ...
        // Agregar a initializeComponents() en constitucion.js
        function setupEnhancedPolling() {
            ...
            async function checkForUpdates() {
                try {
                    ...
                    const response = await fetch(`/api/constitucion/last_update?timestamp=${lastUpdate}`);
            ...
            async function checkFileStatus() {
                try {
                    const response = await fetch('/api/constitucion/file_status');
    ...
    function loadEtiquetaOptions() {
        try {
            ...
            // Agregar etiquetas desde la constante ETIQUETAS_CONSTITUCION
            ETIQUETAS_CONSTITUCION.forEach(etiqueta => {
    ...
    function toggleFixDate() {
        ...
        if (dateFixed) {
            localStorage.setItem('constitucionFixedDate', currentControlDate);
            localStorage.setItem('constitucionDateFixed', 'true');
            document.getElementById('controlDate').setAttribute('disabled', 'disabled');
        } else {
            localStorage.removeItem('constitucionFixedDate');
            localStorage.setItem('constitucionDateFixed', 'false');
    ...
    async function loadValidationData(dateStr) {
        try {
            ...
            // Cargar datos de constitucion sin filtros - para verificar todos los registros
            const constitucionResponse = await fetch(`/api/constitucion/data`);
            if (!constitucionResponse.ok) {
                throw new Error('Error al cargar los datos de constitución');
            }
            const constitucionData = await constitucionResponse.json();
            
            // Cargar TODOS los pendientes, no solo los de constitucion
            ...
            if (pendingResponse.ok) {
                ...
                const specificPendingResponse = await fetch(`/api/pendientes/constitucion?fecha=${formattedDate}`);
                if (specificPendingResponse.ok) {
                    pendingData = await specificPendingResponse.json();
                }
            }
            
            // Filtrar los pendientes por zona constitucion
            pendingData = pendingData.filter(item => item.zona === 'CONSTITUCION');
            ...
            for (const etiqueta of ETIQUETAS_CONSTITUCION) {
                const turnos = ETIQUETA_TURNOS_MAP[etiqueta] || ["UNI"]; // Usar los turnos mapeados o UNI por defecto
                
                for (const turno of turnos) {
                    // Buscar registros OK en constitucion.json
                    const registrosOK = constitucionData.filter(item => 
    ...
    async function loadUserOptions() {
        try {
            const response = await fetch('/api/constitucion/users');
            if (!response.ok) {
    ...
    async function loadTableData() {
        try {
            ...
            const response = await fetch(`/api/constitucion/data?${params.toString()}`);
    ...
    async function openEditModal(id) {
        try {
            ...
            const response = await fetch(`/api/constitucion/record/${id}`);
    ...
    async function saveEditedRecord() {
        try {
            ...
            const response = await fetch('/api/constitucion/update', {
                method: 'POST',
                headers: {
