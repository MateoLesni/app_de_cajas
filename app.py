...
def remesas_guardar_lote():
    ...
    try:
        sql = """
          ...
          VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'Local',NOW(),'revision')
        """
        inserted = 0
        for t in remesas:
...
def _sum_tips_tarjetas_breakdown(cur, table_name, fecha, local):
    ...
    """
    # IMPORTANTE: Solo incluir versión sin acento porque COLLATE utf8mb4_unicode_ci
    # ya hace matching insensible a acentos. Si incluimos ambas variantes se duplican los montos.
    marcas = [
    ...
    try:
        ...
        try:
            cur.execute("""
                ...
                WHERE DATE(cc.fecha)=%s AND cc.local=%s AND cc.estado='ok'
                ORDER BY cc.id
            """, (f, local))
...
def api_tesoreria_resumen_por_local():
    ...
    try:
        ...
        for local in todos_locales:
            ...
            # Si hoy es 13/01, muestra remesas anteriores a 12/01 en estado 'Local' (no retiradas)
            # IMPORTANTE: Solo remesas en ARS (excluye USD)
            fecha_ayer = hoy - timedelta(days=1)
            cur.execute("""
                SELECT SUM(monto) as saldo_historico
                FROM remesas_trns
            ...
            # Si hoy es 13/01, muestra remesas de 12/01 en estado 'Local' (físicamente en el local)
            # IMPORTANTE: Solo remesas en ARS (excluye USD)
            cur.execute("""
                SELECT SUM(monto) as saldo_pendiente
                FROM remesas_trns
